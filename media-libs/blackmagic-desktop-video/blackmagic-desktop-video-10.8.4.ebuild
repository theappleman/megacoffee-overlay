...
DESKTOP_VIDEO_VERSION="10.8.4a4"
...
pkg_pretend() {
	if kernel_is -gt 4 7 && ! use patch_compat; then
		...
		ewarn ""
		ewarn " 1) Recommended: downgrade to an officially supported kernel version (e.g. 4.7)"
		ewarn ""
...
src_prepare() {
	if use patch_compat; then
		epatch "${FILESDIR}/10_8_4__4_9_get_user_pages.patch"
	fi
}
